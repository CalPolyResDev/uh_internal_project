...
class PrinterRequestsRouter(object):
    ...
    EXCLUDED_MODEL = "inventoryemail"

    def db_for_read(self, model, **hints):
        """Routes database read requests to the printer request database."""

        if model._meta.app_label == self.APP_NAME and model._meta.name != self.EXCLUDED_MODEL:
    ...
    def allow_relation(self, obj1, obj2, **hints):
        ...
        if (obj1._meta.app_label == self.APP_NAME or obj2._meta.app_label == self.APP_NAME) and obj1._meta.name != self.EXCLUDED_MODEL and obj2._meta.name != self.EXCLUDED_MODEL:
            return True
        return None
    ...
    def allow_migrate(self, db, app_label, model=None, **hints):
        ...
        if app_label == self.APP_NAME and model._meta.name != self.EXCLUDED_MODEL:
            return False
        return None
